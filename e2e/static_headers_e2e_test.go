...
func runStaticHeadersTransportCase(t *testing.T, useUnixMCP bool) {
	t.Helper()
	...
	const (
		...
		unixAuthServerURL = "http://static-headers-auth"
	)
	...
	if !useUnixMCP {
		...
		authServerURL = unixAuthServerURL
		mcpOptions = append(mcpOptions, mockmcpserver.WithHostHandler("static-headers-auth", authHandler))
	}
	mcpOptions = append(mcpOptions, mockmcpserver.WithProtectedResourceMetadata(oauthex.ProtectedResourceMetadata{
	...
	harnessOptions := []harnesspkg.HarnessOption{
		...
		harnesspkg.WithMCPOptions(mcpOptions...),
	}
	if useUnixMCP {
		harnessOptions = append(harnessOptions, harnesspkg.WithUnixMCP())
	}
	h := harnesspkg.NewHarness(t, harnessOptions...)
