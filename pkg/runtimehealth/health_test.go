...
func TestBuildHealthURLRejectsNonTCPAddr(t *testing.T) {
	...
	_, err := buildHealthURL(&runtimeconfig.HealthConfig{ListenAddr: ":0"}, fakeAddr{})
	require.Error(t, err)
...
func TestBuildHealthURLWithUnixSocket(t *testing.T) {
	...
	healthURL, err := buildHealthURL(&runtimeconfig.HealthConfig{UnixSocket: socketPath}, fakeAddr{})
...
func TestListenHealthUnixSocketRejectsNonSocketPath(t *testing.T) {
	...
	_, err := listenHealth(&runtimeconfig.HealthConfig{UnixSocket: socketPath})
	require.Error(t, err)
...
func TestListenHealthUnixSocketRemovesStaleSocket(t *testing.T) {
	...
	listener, err := listenHealth(&runtimeconfig.HealthConfig{UnixSocket: socketPath})
	require.NoError(t, err)
...
func TestReadinessHandler(t *testing.T) {
	...
	t.Run("ReadyWhenStateNil", func(t *testing.T) {
		...
		readinessHandler(nil, nil, nil)(rec, req)
	...
	t.Run("NotReadyWhenPending", func(t *testing.T) {
		...
		readinessHandler(state, nil, nil)(rec, req)
	...
	t.Run("NotReadyWhenMCPStartupProbePending", func(t *testing.T) {
		...
		readinessHandler(oauthState, probeState, nil)(rec, req)
...
func TestReadinessHandlerReportsCloudflaredPending(t *testing.T) {
	...
	state := fakeReadinessGate{ready: false, reason: "cloudflared startup pending"}
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/readyz", nil)

	readinessHandler(nil, nil, []ReadinessGate{state})(rec, req)
...
func (g fakeReadinessGate) Readiness() (bool, string) {
...
func mustBuildHealthURL(t *testing.T, listenAddr string, addr net.Addr) string {
	...
	healthURL, err := buildHealthURL(&runtimeconfig.HealthConfig{ListenAddr: listenAddr}, addr)
	require.NoError(t, err)
	return healthURL
}
