// Package runtimehealth owns the health-only HTTP surface shared by the
// customer runtime binaries. It deliberately does not register admin or UI
// routes.
...
// ReadinessGate lets an optional runtime extension participate in /readyz
// without making runtimehealth import that extension.
...
func (s *healthService) markBound() {
	...
	s.boundMu.Do(func() { close(s.boundCh) })
}
...
func newHealthService(p healthParams) (*healthService, error) {
	if p.AdminMux == nil {
		return nil, errors.New("health: admin mux is required")
	}
	if p.HealthConfig == nil {
		return nil, errors.New("health: config is required")
	}
	if p.Logger == nil {
		return nil, errors.New("health: logger is required")
	}
	if p.MeterProvider == nil {
		return nil, errors.New("health: meter provider is required")
	}
	if p.MetricExporter == nil {
		return nil, errors.New("health: metric exporter is required")
	}
	...
	livenessGauge, err := meter.Int64ObservableGauge("liveness", metric.WithDescription("Reports 1 when the tunnel client process is live"))
	if err != nil {
		return nil, fmt.Errorf("create liveness gauge: %w", err)
	}
	readinessGauge, err := meter.Int64ObservableGauge("readiness", metric.WithDescription("Reports 1 when the tunnel client reports readiness"))
	if err != nil {
	...
	p.Lifecycle.Append(fx.Hook{
		OnStart: func(ctx context.Context) error {
			_, err := meter.RegisterCallback(func(_ context.Context, observer metric.Observer) error {
				...
				if isReady(p.OAuthState, p.MCPProbeState, p.ReadinessGates) {
...
func listenHealth(cfg *runtimeconfig.HealthConfig) (net.Listener, error) {
	if cfg == nil {
		return nil, errors.New("health: config is required")
	}
	if cfg.UnixSocket != "" {
...
func buildHealthURL(cfg *runtimeconfig.HealthConfig, addr net.Addr) (string, error) {
...
func isReady(oauthState *oauth.DiscoveryState, probeState *mcpclient.ProbeState, gates []ReadinessGate) bool {
	status, _ := readinessStatus(oauthState, probeState, gates)
...
func readinessHandler(oauthState *oauth.DiscoveryState, probeState *mcpclient.ProbeState, gates []ReadinessGate) http.HandlerFunc {
	return func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "text/plain; charset=utf-8")
		statusCode, body := readinessStatus(oauthState, probeState, gates)
		w.WriteHeader(statusCode)
		_, _ = w.Write([]byte(body))
	}
}

func readinessStatus(oauthState *oauth.DiscoveryState, probeState *mcpclient.ProbeState, gates []ReadinessGate) (int, string) {
	for _, gate := range gates {
		if gate == nil {
			continue
		}
		if ready, reason := gate.Readiness(); !ready {
			if strings.TrimSpace(reason) == "" {
				reason = "runtime dependency is not ready"
			}
	...
	if probeState != nil && probeState.IsDone() {
		if _, err, ok := probeState.Wait(10 * time.Millisecond); ok && err != nil {
			...
			if mcpclient.IsStartupWaitTimeoutError(err) {
				return http.StatusServiceUnavailable, "mcp startup wait failed: " + sanitizeReadinessError(err)
			}
