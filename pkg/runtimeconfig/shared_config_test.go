...
func LoadRuntimeForTest(args []string, lookupEnv func(string) (string, bool)) (*Config, error) {
	return Load(args, FlavorRuntime, lookupEnv)
}
...
`)
	...
	cfg, err := LoadRuntimeForTest([]string{"--config", configPath}, lookupEnvMap(map[string]string{
...
`,
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := LoadRuntimeForTest([]string{"--config", configPath}, lookupEnvMap(map[string]string{
				"YAML_CONTROL_PLANE_API_KEY": "yaml-control-key",
			}))
...
`)
	...
	cfg, err := LoadRuntimeForTest(nil, lookupEnvMap(map[string]string{
...
`)
	...
	_, err := LoadRuntimeForTest([]string{"--config", configPath}, lookupEnvMap(nil))
	if err == nil {
...
func TestLoadCABundleRejectsInvalidPEM(t *testing.T) {
	...
	_, err := LoadRuntimeForTest(args, lookupEnvMap(map[string]string{
		"CONTROL_PLANE_API_KEY": "control-key",
	}))
	if err == nil {
...
func TestLoadRequiresTunnelID(t *testing.T) {
	...
	_, err := LoadRuntimeForTest(nil, cfgLookup)
	if err == nil {
...
`)
	cfg, err := LoadRuntimeForTest([]string{"--config", path}, lookupEnvMap(map[string]string{"OPENAI_API_KEY": "key"}))
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if got := cfg.ControlPlane.PollChannels; len(got) != 2 || got[0] != types.ChannelHarpoon || got[1] != types.DefaultChannel {
