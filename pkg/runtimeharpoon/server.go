...
func FilterOutboundHeaders(headers map[string]string) (http.Header, int, []string) {
	return filterOutboundHeaders(headers)
}
...
func IsBlockedOutboundHeader(headerName string) bool {
	return isBlockedOutboundHeader(headerName)
}
...
// ResponseContentTypeForLog returns the normalized bounded content type used
// in call receipts.
func ResponseContentTypeForLog(contentType string) string {
	return responseContentTypeForLog(contentType)
}
...
func (s *Server) normalizeMaxResponseBytes(value *int) (int, error) {
	...
	if limit <= 0 {
		limit = runtimeconfig.DefaultHarpoonMaxResponseBytes
	}
	if value == nil {
		return limit, nil
	}
...
func (s *Server) normalizeRedirects(followRedirects *bool, maxRedirects *int) (int, bool, error) {
	...
	if limit <= 0 {
		limit = runtimeconfig.DefaultHarpoonMaxRedirects
	}
	if maxRedirects == nil {
...
func buildCallTargetSchema(cfg *runtimeconfig.HarpoonConfig) *jsonschema.Schema {
...
func buildCallTargetOutputSchema(cfg *runtimeconfig.HarpoonConfig) *jsonschema.Schema {
	reflector := &jsonschema.Reflector{DoNotReference: true}
...
func applyCallTargetSchemaBounds(schema *jsonschema.Schema, cfg *runtimeconfig.HarpoonConfig) {
	if schema == nil || schema.Properties == nil {
		return
	}
	maxResponseBytes := runtimeconfig.DefaultHarpoonMaxResponseBytes
	if cfg != nil && cfg.MaxResponseBytes > 0 {
		maxResponseBytes = cfg.MaxResponseBytes
	}
	maxRedirects := runtimeconfig.DefaultHarpoonMaxRedirects
...
func applyCallTargetOutputSchemaBounds(schema *jsonschema.Schema, cfg *runtimeconfig.HarpoonConfig) {
	if schema == nil || schema.Properties == nil {
		return
	}
	maxResponseBytes := runtimeconfig.DefaultHarpoonMaxResponseBytes
...
func (s *Server) recordCall(input callRecordInput) {
	...
	event := CallEvent{
		...
		Method:                  input.method,
		Status:                  input.status,
		RequestBody:             input.params.Body,
		RequestBytes:            input.reqBytes,
		ResponseBytes:           input.respBytes,
		Error:                   input.errorMsg,
		StartedAt:               input.startedAt,
		ResponseContentType:     input.responseContentType,
		ResponseBody:            bytes.Clone(input.responseBody),
		ResponseBodyTransformed: bytes.Clone(input.responseBodyTransformed),
	}
